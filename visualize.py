...
# Load results
df = pd.read_csv("C:\\CPSC481\\results.csv")

# Model display order (best to worst quality)
model_order = ["deepseek-f16", "deepseek-q8", "deepseek-q6k", "deepseek-q4km", "deepseek-q2k"]
labels = ["F16", "Q8_0", "Q6_K", "Q4_K_M", "Q2_K"]
file_sizes = [16.07, 8.54, 6.60, 4.92, 3.18]  # GB
...
plt.title("Accuracy per Quantization Level", fontsize=16)
plt.xlabel("Model")
plt.ylabel("Accuracy (%)")
plt.ylim(0, 100)
...
plt.tight_layout()
plt.savefig("C:\\CPSC481\\graph_accuracy.png", dpi=150)
plt.close()
...
plt.title("Average Response Time per Quantization Level", fontsize=16)
plt.xlabel("Model")
...
plt.tight_layout()
plt.savefig("C:\\CPSC481\\graph_time.png", dpi=150)
plt.close()
...
plt.title("File Size vs Accuracy Tradeoff", fontsize=16)
plt.xlabel("File Size (GB)")
plt.ylabel("Accuracy (%)")
plt.tight_layout()
plt.savefig("C:\\CPSC481\\graph_size_vs_accuracy.png", dpi=150)
plt.close()
print("Saved graph_size_vs_accuracy.png")

print("\nAll 3 graphs saved to C:\\CPSC481")